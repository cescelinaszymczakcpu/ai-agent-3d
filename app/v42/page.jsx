...
import './v42.css';
import {LivingOrb,presets,voicePresets,hairStyles,eyeStyles,mustacheStyles} from './LivingOrb';
...
export default function LivingOrbCreator(){
  ...
  return <main className="v42">
    <div className="v42Head"><div><div className="eyebrow">ALTER · LIVING ORB 4.2</div><h1>Living Orb Creator</h1><p>Modułowa postać AI: szkło, światło, cienkie kończyny i Voice Orb zamiast klasycznego lip-sync.</p></div><button className="miniButton" onClick={()=>location.href='/'}>← Aplikacja</button></div>
    <div className="creatorGrid">
      <section className="orbStage">
        <LivingOrb config={config} state={state} audioLevel={audioLevel} large/>
      ...
      <aside className="editorPanel">
        ...
        <Section title="Character"><div className="seg">{['female','male','neutral'].map(g=><button key={g} className={config.gender===g?'active':''} onClick={()=>set('gender',g)}>{g}</button>)}</div><Slider label="Grubość kończyn" value={config.limbWidth} min=.45 max=1.25 step=.05 onChange={v=>set('limbWidth',v)}/><Slider label="Długość rąk" value={config.armLength} min=.75 max=1.25 step=.05 onChange={v=>set('armLength',v)}/><Slider label="Długość nóg" value={config.legLength} min=.8 max=1.3 step=.05 onChange={v=>set('legLength',v)}/></Section>
        <Section title="Orb"><Color label="Primary" value={config.primary} onChange={v=>set('primary',v)}/><Color label="Secondary" value={config.secondary} onChange={v=>set('secondary',v)}/><Color label="Accent" value={config.accent} onChange={v=>set('accent',v)}/><Slider label="Przezroczystość" value={config.opacity} min=.5 max=1 step=.02 onChange={v=>set('opacity',v)}/><Slider label="Glow" value={config.glow} min=.1 max=1 step=.05 onChange={v=>set('glow',v)}/><Slider label="Glass" value={config.glass} min=.2 max=1 step=.05 onChange={v=>set('glass',v)}/></Section>
        <Section title="Face"><Tog label="Oczy" checked={config.eyes} onChange={v=>set('eyes',v)}/><Select label="Styl oczu" value={config.eyeStyle} values={eyeStyles} onChange={v=>set('eyeStyle',v)}/><Color label="Kolor oczu" value={config.eyeColor} onChange={v=>set('eyeColor',v)}/><Tog label="Usta" checked={config.mouth} onChange={v=>set('mouth',v)}/></Section>
        ...
        <Section title="Voice Orb"><Select label="Preset" value={config.voicePreset} values={voicePresets} onChange={v=>set('voicePreset',v)}/><Slider label="Reakcja audio" value={audioLevel} min=.05 max=1 step=.05 onChange={setAudioLevel}/><div className="toggleRow"><span className="toggle">Audio reactive: ON</span><span className="toggle">Lip-sync: OFF</span></div></Section>
      </aside>
    </div>
  </main>
}

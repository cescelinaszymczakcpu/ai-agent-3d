...
function Limb({side=1,kind='arm',config}){
  ...
  const armL=config.armLength||1, legL=config.legLength||1;
  const pts=isArm
...
function Eye({side,config,blink}){
  const s=side; const scale=(config.eyeSize||1);
  return <group position={[s*.39,.15,.87]} scale={[scale,scale*Math.max(.06,blink),scale]}>
...
function OrbCore({config,state,audioLevel}){
  ...
  const blinkRef=useRef({next:3+Math.random()*4,until:0,double:false});
  ...
  return <group ref={group}>
    <mesh ref={shell}>
      ...
      <meshPhysicalMaterial {...glassMaterial(config.primary,config.opacity||.78,.045,.12,.83)} thickness={2.4} ior={1.38} clearcoat={1} clearcoatRoughness={.02} attenuationColor={config.secondary} attenuationDistance={1.7}/>
    </mesh>
    <mesh ref={inner} scale={.9}>
      <sphereGeometry args={[1,72,72]}/>
      <meshPhysicalMaterial color={config.secondary} transparent opacity={.17} roughness={.14} transmission={.25} emissive={config.accent} emissiveIntensity={.24}/>
    </mesh>
    <mesh ref={energy} scale={.58}>
      <icosahedronGeometry args={[1,5]}/>
      <meshPhysicalMaterial color={config.accent} transparent opacity={.20} roughness={.1} transmission={.35} emissive={config.accent} emissiveIntensity={.8} wireframe={false}/>
    </mesh>
    ...
    {config.eyes && config.eyeStyle!=='none' && <><Eye side={-1} config={config} blink={blinkValue.current}/><Eye side={1} config={config} blink={blinkValue.current}/></>}
...
function Scene({config,state,audioLevel}){
  return <>
    ...
    <directionalLight position={[0,3,5]} intensity={2.2} color="#dff8ff"/>
    <pointLight position={[-4,1,3]} intensity={30} distance={9} color="#19dfff"/>
    <pointLight position={[4,1,2]} intensity={28} distance={9} color="#b04cff"/>
    <pointLight position={[0,-3,2]} intensity={18} distance={7} color="#ff5dbd"/>
    <Float speed={.6} rotationIntensity={.04} floatIntensity={.08}>
      <group position={[0,.78,0]}>
        <OrbCore config={config} state={state} audioLevel={audioLevel}/>
        <Limb side={-1} kind="arm" config={config}/><Limb side={1} kind="arm" config={config}/>
        <Limb side={-1} kind="leg" config={config}/><Limb side={1} kind="leg" config={config}/>
      </group>
    </Float>
...
export function LivingOrb({config,state='idle',audioLevel=.12}){
  return <div className="livingOrbCanvas" aria-label="Living Orb 3D">
    <Canvas camera={{position:[0,.1,6.1],fov:37}} dpr={[1,2]} gl={{antialias:true,alpha:true,powerPreference:'high-performance'}}>
      <Scene config={config} state={state} audioLevel={audioLevel}/>
    </Canvas>
  </div>
}

...
export default function LivingOrbCreatorPage(){
 ...
 const clearMajor=keep=>{if(keep!=="bow")patchBow({type:"none"});if(keep!=="hat")patchHat({type:"none"});if(keep!=="audio")patchAudio({type:"none"});if(keep!=="glasses")patchGlasses({type:"none"})};
 const selectHair=style=>{const profile=hairProfile(style);setHair(prev=>({...prev,style}));patchHat({hairStyle:profile});patchAudio({hairStyle:profile});patchGlasses({hairStyle:profile})};
 ...
 return <main className="creatorRoot" style={{background:"#020611"}}>
  <section className="creatorPreview" style={{background:"#020611"}}>
   {characterType==="orb"&&<LivingLumeniaOrb emotion={emotion} view={view} colorFlowSpeed={flow} orbColors={colors} state={state} audioLevel={realtime.audioLevel} audioBands={realtime.audioBands} hair={hair} bow={bow} hat={hat} audio={audioAccessory} glasses={glasses} style={{width:"100%",height:"100%",minHeight:"100%",background:"#020611"}}/>}
  ...
  <aside className="creatorPanel">
   ...
   <div className="stageNote">NAPRAWA + ETAP 8/9: swobodny OrbitControls po wybraniu widoku · error boundary dla opcjonalnych warstw · CAT EYE · ONE/TWO/THREE STRANDS · SIDE STRAND · SOFT CURLS · MESSY STRANDS · SIDE PONYTAIL · TWIN PONYTAILS. Włosy mogą działać równocześnie z jednym aktualnie testowanym dodatkiem głównym; kolejne kombinacje odblokujemy po resolverze accessory-vs-accessory.</div>
  </aside>
 </main>;
}

...
const AUDIO_TYPES=["none","earmuffs"];
...
export default function LivingOrbCreatorPage(){
 ...
 const selectBow=v=>{patchBow({type:v});if(v!=="none"){patchHat({type:"none"});patchAudio({type:"none"});}};
 const selectHat=v=>{patchHat({type:v,color:v==="top-hat"?"#765BFF":"#8E72FF",offsetX:0,offsetY:0,offsetZ:0,rotationX:0,rotationY:0,rotationZ:0});if(v!=="none"){patchBow({type:"none"});patchAudio({type:"none"});}};
 const selectAudio=v=>{patchAudio({type:v,offsetX:0,offsetY:0,offsetZ:0,rotationX:0,rotationY:0,rotationZ:0});if(v!=="none"){patchBow({type:"none"});patchHat({type:"none"});}};

 return <main className="creatorRoot" style={{background:"#020611"}}>
  <section className="creatorPreview" style={{background:"#020611"}}>
   {characterType==="orb"&&<LivingLumeniaOrb emotion={emotion} view={view} colorFlowSpeed={flow} orbColors={colors} state={state} audioLevel={realtime.audioLevel} audioBands={realtime.audioBands} bow={bow} hat={hat} audio={audioAccessory} style={{width:"100%",height:"100%",minHeight:"100%",background:"#020611"}}/>}
   <div className="creatorBadge">SAFE BODY · BOWS + HATS + EARMUFFS · {state.toUpperCase()}</div>
  ...
  <aside className="creatorPanel">
   ...
   <p className="creatorIntro">Stabilny Living Orb, breathing, color flow i bezpieczny rig kończyn pozostają bazą. Dodatki są ponownie włączane pojedynczo i każdy ma własne zabezpieczenie kolizji.</p>
   ...
   <div className="stageNote">EARMUFF TEST: FRONT / LEFT / RIGHT / BACK / TOP · NONE / SHORT / REFERENCE CURL / LONG HAIR · skrajne SCALE/OFFSET/ROTATION · brak penetracji orba podczas breathing. BOW / HAT / EARMUFFS są chwilowo wzajemnie wykluczone do czasu resolvera kolizji między różnymi dodatkami.</div>
  </aside>
 </main>;
}

...
export default function LivingOrbCreatorPage(){
 ...
 const [hat,setHat]=useState({type:"none",material:"iridescent",color:"#8E6CFF",scale:1,rotation:0,rotationX:0,rotationY:0,rotationZ:0,height:0,offsetX:0,offsetY:0,offsetZ:0,collisionRadius:.59});
 ...
 return <main className="creatorRoot">
  <section className="creatorPreview"><LivingLumeniaOrb variant={variant} emotion={emotion} emotionIntensity={1} state={effectiveState} audioLevel={realtime.audioLevel} audioBands={realtime.audioBands} gesturesEnabled lipSyncEnabled hairStyle={hairStyle} flyaways={flyaways} mustacheStyle={variant==="male"?mustacheStyle:"none"} beardStyle={variant==="male"?beardStyle:"none"} hat={hat} glasses={glasses} headAccessory={headAccessory} headDecoration={headDecoration} view={view} colorFlowSpeed={flow} orbColors={colors} style={{width:"100%",height:"100%",minHeight:"100%"}}/><div className="creatorBadge">STAGE 1 · BERET + HEAD ANCHORS · {emotion.toUpperCase()}</div><div className="viewDock"><button onClick={()=>setView("front")} className={view==="front"?"mini active":"mini"}>FRONT</button><button onClick={()=>setView("left")} className={view==="left"?"mini active":"mini"}>LEFT</button><button onClick={()=>setView("right")} className={view==="right"?"mini active":"mini"}>RIGHT</button><button onClick={()=>setView("back")} className={view==="back"?"mini active":"mini"}>BACK</button><button onClick={()=>setView("top")} className={view==="top"?"mini active":"mini"}>TOP</button></div></section>
  <aside className="creatorPanel">
   <div className="creatorHead"><div><span className="creatorEyebrow">SECOND LIFE · REPAIR STAGE 1</span><h1>Create Your Orb</h1></div><a className="backLink" href="/v42">ORB</a></div>
   <p className="creatorIntro">Teraz naprawiany jest tylko system anchorów i beret. Nowy beret ma pivot na dolnej krawędzi, customową geometrię z grubością oraz automatyczny test penetracji z orbem.</p>
   ...
   <div className="controlBlock"><div className="controlTitle">BERET HAIR FIT TEST</div>{variant==="female"?<Chips items={["none","short","elegant","long-wave"]} value={hairStyle} onChange={setHairStyle}/>:<Chips items={["none","swept"]} value={hairStyle} onChange={setHairStyle}/>}<p className="controlHelp">NO HAIR = NONE · SHORT HAIR = SHORT · REFERENCE CURL = ELEGANT · LONG HAIR = LONG WAVE.</p><div className="subTitle">FLYAWAYS</div><Chips items={FLY} value={flyaways.type} onChange={v=>setFlyaways({type:v})}/></div>
   ...
   <div className="controlBlock"><div className="controlTitle">HATS · STAGE 1 BERET</div><AccessoryGrid items={HATS} value={hat.type} onChange={v=>patchHat({type:v,height:0,offsetX:0,offsetY:0,offsetZ:0,rotationX:0,rotationY:0,rotationZ:0,collisionRadius:.59})} color={hat.color} mode={hat.material}/><div className="subTitle">MATERIAL</div><Chips items={MATERIALS} value={hat.material} onChange={v=>patchHat({material:v})}/><ColorControl label="COLOR" value={hat.color} onChange={v=>patchHat({color:v})}/><Range label="SCALE" value={hat.scale} min={.82} max={1.18} step={.01} onChange={v=>patchHat({scale:v})}/>{hat.type==="beret"&&<><div className="subTitle">BERET FIT / TRANSFORM</div><Range label="OFFSET X" value={hat.offsetX} min={-.30} max={.30} step={.01} onChange={v=>patchHat({offsetX:v})}/><Range label="OFFSET Y" value={hat.offsetY} min={-.22} max={.30} step={.01} onChange={v=>patchHat({offsetY:v})}/><Range label="OFFSET Z" value={hat.offsetZ} min={-.30} max={.30} step={.01} onChange={v=>patchHat({offsetZ:v})}/><Range label="ROTATION X" value={hat.rotationX} min={-.35} max={.35} step={.01} onChange={v=>patchHat({rotationX:v})}/><Range label="ROTATION Y" value={hat.rotationY} min={-.50} max={.50} step={.01} onChange={v=>patchHat({rotationY:v})}/><Range label="ROTATION Z" value={hat.rotationZ} min={-.45} max={.45} step={.01} onChange={v=>patchHat({rotationZ:v})}/><Range label="COLLISION RADIUS" value={hat.collisionRadius} min={.48} max={.74} step={.01} onChange={v=>patchHat({collisionRadius:v})}/><p className="controlHelp">Przesunięcie w dół nie może wcisnąć beretu w orb: dolny pierścień jest próbkowany w 20 punktach i przy penetracji pozycja Y jest automatycznie wypychana na zewnątrz.</p></>}</div>

   <div className="controlBlock"><div className="controlTitle">HEAD DECORATIONS · EXISTING</div><AccessoryGrid items={DECOR} value={headDecoration.type} onChange={v=>setHeadDecoration(p=>({...p,type:v}))} color={headDecoration.color} mode={headDecoration.material}/><div className="subTitle">MATERIAL</div><Chips items={MATERIALS} value={headDecoration.material} onChange={v=>setHeadDecoration(p=>({...p,material:v}))}/><ColorControl label="COLOR" value={headDecoration.color} onChange={v=>setHeadDecoration(p=>({...p,color:v}))}/><Range label="SCALE" value={headDecoration.scale} min={.72} max={1.28} step={.02} onChange={v=>setHeadDecoration(p=>({...p,scale:v}))}/></div>

   <div className="controlBlock"><div className="controlTitle">EYEWEAR · EXISTING</div><AccessoryGrid items={EYEWEAR} value={glasses.type} onChange={v=>patchGlasses({type:v})} color={glasses.frameColor}/><ColorControl label="FRAME" value={glasses.frameColor} onChange={v=>patchGlasses({frameColor:v})}/><ColorControl label="LENS" value={glasses.lensColor} onChange={v=>patchGlasses({lensColor:v})}/><Range label="FRAME THICKNESS" value={glasses.frameThickness} min={.016} max={.045} step={.001} onChange={v=>patchGlasses({frameThickness:v})}/><Range label="SCALE" value={glasses.scale} min={.85} max={1.18} step={.01} onChange={v=>patchGlasses({scale:v})}/></div>

   <div className="controlBlock"><div className="controlTitle">AUDIO ACCESSORIES · EXISTING</div><AccessoryGrid items={AUDIO} value={headAccessory.type} onChange={v=>setHeadAccessory(p=>({...p,type:v}))} color={headAccessory.color} mode={headAccessory.material}/><div className="subTitle">MATERIAL</div><Chips items={MATERIALS} value={headAccessory.material} onChange={v=>setHeadAccessory(p=>({...p,material:v}))}/><ColorControl label="COLOR" value={headAccessory.color} onChange={v=>setHeadAccessory(p=>({...p,color:v}))}/></div>
   ...
   <div className="stageNote">ETAP 1 QA: BERET → NONE / SHORT / ELEGANT / LONG WAVE → FRONT / LEFT / RIGHT / BACK / TOP. Dopiero po zaliczeniu tego etapu przechodzimy do kokardek.</div>
  </aside>
 </main>;
}

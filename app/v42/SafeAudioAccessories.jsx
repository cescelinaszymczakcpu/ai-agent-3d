...
function createHeadphoneCupGeometry(){
 ...
 s.moveTo(0,.42);s.bezierCurveTo(.25,.42,.32,.28,.32,.05);s.bezierCurveTo(.32,-.25,.20,-.42,0,-.44);s.bezierCurveTo(-.20,-.42,-.32,-.25,-.32,.05);s.bezierCurveTo(-.32,.28,-.25,.42,0,.42);
 ...
 g.center();g.rotateY(Math.PI/2);g.computeVertexNormals();return g;
}
...
function padSamples(side,x,y,z,size,innerDepth,ry,rz){const innerX=x-side*innerDepth*size;const pts=[];for(let i=0;i<28;i+=1){const a=i/28*Math.PI*2;pts.push(new THREE.Vector3(innerX,y+Math.cos(a)*ry*size,z+Math.sin(a)*rz*size))}pts.push(new THREE.Vector3(innerX,y,z));return pts}
...
function rotatedWorstPenetration({leftX,rightX,cupY,cupZ,size,rotation,safety,innerDepth,padRy,padRz,curve}){
 ...
 let worst=0;for(const source of points){const p=source.clone().applyQuaternion(q);const need=MAX_BREATH_RADIUS+safety-p.distanceTo(ORB_CENTER);if(need>worst)worst=need}return worst;
}

function resolveAudioGeometry(audio={},type="earmuffs"){
 const fullCup=type==="headphones"||type==="headset";
 const size=THREE.MathUtils.clamp(audio.scale??1,.78,1.20);
 const hair=HAIR_CLEARANCE[audio.hairStyle||"none"]??0;
 const cupY=(fullCup?.46:.50)+(audio.offsetY||0);
 const cupZ=.02+(audio.offsetZ||0);
 const offsetX=THREE.MathUtils.clamp(audio.offsetX||0,-.16,.16);
 const innerDepth=fullCup?.245:.205;
 const padRy=fullCup?.39:.405;
 const padRz=fullCup?.30:.305;
 const safety=Math.max(.035,audio.collisionRadius??(fullCup?.060:.055))+hair;
 const required=sphereHalfWidth(cupY,cupZ)+innerDepth*size+safety;
 const natural=(fullCup?1.94:1.84)*size;
 let leftX=Math.min(-required,-natural)+offsetX,rightX=Math.max(required,natural)+offsetX;
 if(leftX>-required)leftX=-required;if(rightX<required)rightX=required;
 let topY=ORB_CENTER.y+MAX_BREATH_RADIUS+(fullCup?.22:.18)+hair;
 ...
 const makeCurve=fullCup?makeHeadphoneBandCurve:makeEarmuffBandCurve;
 for(let pass=0;pass<7;pass+=1){const curve=makeCurve(leftX,rightX,cupY,cupZ,topY,size);const worst=rotatedWorstPenetration({leftX,rightX,cupY,cupZ,size,rotation,safety:safety*.65,innerDepth,padRy,padRz,curve});if(worst<=.001)break;leftX-=worst*.94+.025;rightX+=worst*.94+.025;topY+=worst*.64+.018}
 return {size,leftX,rightX,cupY,cupZ,topY,rotation,safety};
}

function FullCupAssembly({g,material,color}){
...
function outside(p,margin=.08){const v=p.clone().sub(ORB_CENTER);const min=MAX_BREATH_RADIUS+margin;if(v.length()<min)v.normalize().multiplyScalar(min).add(ORB_CENTER);return p.copy(v.length()===0?new THREE.Vector3(0,0,min):v)}
function boomCurve(g){
 const raw=[new THREE.Vector3(g.rightX-.18*g.size,g.cupY-.06*g.size,.22),new THREE.Vector3(1.68*g.size,.22,.64),new THREE.Vector3(1.24*g.size,.12,1.10),new THREE.Vector3(.92*g.size,.09,1.50)];
 const pts=raw.map(p=>{const radial=p.clone().sub(ORB_CENTER);const min=MAX_BREATH_RADIUS+g.safety*.8;if(radial.length()<min)p.copy(ORB_CENTER).add(radial.normalize().multiplyScalar(min));return p});
 return new THREE.CatmullRomCurve3(pts,false,"centripetal");
}
...
export function PremiumEarmuffs({audio={}}){
 ...
 return <group rotation={g.rotation}><OvalPad side={-1} x={g.leftX} y={g.cupY} z={g.cupZ} size={g.size} color={color} material={material}/><OvalPad side={1} x={g.rightX} y={g.cupY} z={g.cupZ} size={g.size} color={color} material={material}/><mesh><tubeGeometry args={[curve,128,.072*g.size,14,false]}/><AudioMaterial color={color} mode={material} opacity={.84}/></mesh><mesh><tubeGeometry args={[innerCurve,128,.032*g.size,12,false]}/><AudioMaterial color="#DDF8FF" mode={material} opacity={.62} frosted/></mesh></group>;
}

export function PremiumHeadphones({audio={}}){
 const g=resolveAudioGeometry(audio,"headphones");const material=audio.material||"iridescent",color=audio.color||"#6FE8FF";
 return <group rotation={g.rotation}><FullCupAssembly g={g} material={material} color={color}/></group>;
}

export function PremiumHeadset({audio={}}){
 const g=resolveAudioGeometry(audio,"headset");const material=audio.material||"iridescent",color=audio.color||"#72E9FF";
 const boom=useMemo(()=>boomCurve(g),[g.rightX,g.cupY,g.size,g.safety]);
 const mic=boom.getPoint(1);
 return <group rotation={g.rotation}><FullCupAssembly g={g} material={material} color={color}/><mesh><tubeGeometry args={[boom,92,.030*g.size,10,false]}/><AudioMaterial color="#D7F8FF" mode={material} opacity={.88}/></mesh><mesh position={mic.toArray()} rotation={[0,.18,Math.PI/2]} scale={[.11,.075,.075]}><capsuleGeometry args={[1,.45,7,12]}/><AudioMaterial color="#FF85CE" mode={material} opacity={.92}/></mesh><mesh position={[g.rightX-.12*g.size,g.cupY-.02,.18]} scale={[.095,.095,.095]}><sphereGeometry args={[1,24,18]}/><AudioMaterial color="#FFD47D" mode={material} opacity={.90}/></mesh></group>;
}
...
export default function SafeAudioAccessories({audio=null}){
 ...
 if(audio.type==="headset")return <PremiumHeadset audio={audio}/>;
 return null;
}

...
class AccessoryBoundary extends React.Component{
 constructor(props){super(props);this.state={failed:false}}
 static getDerivedStateFromError(){return {failed:true}}
 componentDidCatch(error){console.error("ALTER optional 3D layer disabled after runtime error",error)}
 render(){return this.state.failed?null:this.props.children}
}

function viewPosition(view){
 if(view==="left")return new THREE.Vector3(-8.8,-.10,0);
 if(view==="right"||view==="side")return new THREE.Vector3(8.8,-.10,0);
 if(view==="back")return new THREE.Vector3(0,-.10,-8.8);
 if(view==="top")return new THREE.Vector3(0,9.1,.20);
 return new THREE.Vector3(0,-.10,8.8);
}

function CameraView({view="front"}){
 const {camera}=useThree();
 const dest=useRef(viewPosition(view));
 const moving=useRef(true);
 const target=useMemo(()=>new THREE.Vector3(0,-.22,0),[]);
 useEffect(()=>{dest.current=viewPosition(view);moving.current=true},[view]);
 useFrame(()=>{
  if(!moving.current)return;
  camera.position.lerp(dest.current,.14);
  camera.lookAt(target);
  if(camera.position.distanceTo(dest.current)<.025){camera.position.copy(dest.current);camera.lookAt(target);moving.current=false}
 });
 return null;
}
...
function LivingOrb({emotion="neutral",orbColors=null,colorFlowSpeed=.06}){
 ...
 const base=useMemo(()=>[new THREE.Vector3(-.42,.28,.12),new THREE.Vector3(.40,.22,-.10),new THREE.Vector3(-.18,-.34,.18),new THREE.Vector3(.30,-.26,.10)],[]);
 ...
 return <group ref={root} position={[0,.35,0]}>
  <group ref={energy}>{palette.map((color,i)=><mesh key={`${color}-${i}`} ref={el=>{lobes.current[i]=el}} position={base[i]}><sphereGeometry args={[1.03,48,48]}/><meshBasicMaterial color={color} transparent opacity={.22} depthWrite={false} blending={THREE.AdditiveBlending}/></mesh>)}<mesh scale={.72}><sphereGeometry args={[1.18,48,48]}/><meshBasicMaterial color="#F5FBFF" transparent opacity={.08} depthWrite={false} blending={THREE.AdditiveBlending}/></mesh></group>
  ...
  <pointLight position={[0,.15,1.3]} color={palette[0]} intensity={1.9*e.glow} distance={5}/><pointLight position={[.8,-.3,-.8]} color={palette[3]} intensity={1.5*e.glow} distance={4}/>
 </group>;
}

function Character({emotion,orbColors,colorFlowSpeed,state,audioLevel,audioBands,bow,hat,audio,glasses,hair}){
 const root=useRef();
 ...
 return <group ref={root}>
  ...
  <AccessoryBoundary key={`glasses-${glasses?.type||"none"}`}><SafeEyewearAccessories glasses={glasses}/></AccessoryBoundary>
 </group>;
}

function StudioLights(){return <><ambientLight intensity={.42}/><hemisphereLight intensity={.62} color="#DFF8FF" groundColor="#050914"/><directionalLight position={[-4,5,5]} color="#DDF9FF" intensity={2.3}/><directionalLight position={[4,2,2]} color="#E3A7FF" intensity={1.6}/><pointLight position={[-3,-1,4]} color="#5AEAFF" intensity={2.2} distance={9}/><pointLight position={[3,-1,3]} color="#FF79C9" intensity={1.9} distance={8}/></>}
...
export default function LivingLumeniaOrb({emotion="neutral",view="front",colorFlowSpeed=.06,orbColors=null,state="idle",audioLevel=0,audioBands=null,bow=null,hat=null,audio=null,glasses=null,hair=null,style={}}){
 return <div style={{width:"100%",height:"100%",minHeight:650,background:"#020611",overflow:"hidden",position:"relative",...style}}>
  <Canvas dpr={[1,1.5]} camera={{position:[0,-.10,8.8],fov:38,near:.1,far:40}} gl={{antialias:true,alpha:false,powerPreference:"high-performance"}} fallback={<div style={{width:"100%",height:"100%",minHeight:650,display:"grid",placeItems:"center",background:"#020611",color:"#DDF7FF",fontFamily:"system-ui"}}>3D renderer unavailable</div>} onCreated={({gl})=>{gl.setClearColor("#020611",1);gl.toneMapping=THREE.ACESFilmicToneMapping;gl.toneMappingExposure=1.16;gl.outputColorSpace=THREE.SRGBColorSpace;gl.domElement.style.display="block";gl.domElement.style.background="#020611"}}>
   <Scene emotion={emotion} view={view} colorFlowSpeed={THREE.MathUtils.clamp(colorFlowSpeed,.01,.15)} orbColors={orbColors} state={state} audioLevel={THREE.MathUtils.clamp(audioLevel||0,0,1)} audioBands={audioBands} bow={bow} hat={hat} audio={audio} glasses={glasses} hair={hair}/>
  </Canvas>
 </div>;
}

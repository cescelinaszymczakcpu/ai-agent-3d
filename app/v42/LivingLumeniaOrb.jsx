...
function Character({emotion,orbColors,colorFlowSpeed,state,audioLevel,audioBands,bow,hat,audio,glasses}){
 const root=useRef();
 ...
 return <group ref={root}>
  ...
  <SafeBowAccessories bow={bow}/><SafeHatAccessories hat={hat}/><SafeAudioAccessories audio={audio}/><SafeEyewearAccessories glasses={glasses}/>
 </group>;
}
...
function Scene(props){return <><color attach="background" args={["#020611"]}/><StudioLights/><CameraView view={props.view}/><Character {...props}/><OrbitControls target={[0,-.22,0]} enablePan={false} enableDamping dampingFactor={.08} minDistance={6.4} maxDistance={11}/></>}
export function RealisticAccessoryModel(){return null;}

export default function LivingLumeniaOrb({emotion="neutral",view="front",colorFlowSpeed=.06,orbColors=null,state="idle",audioLevel=0,audioBands=null,bow=null,hat=null,audio=null,glasses=null,style={}}){
 return <div style={{width:"100%",height:"100%",minHeight:650,background:"#020611",overflow:"hidden",position:"relative",...style}}>
  <Canvas dpr={[1,1.5]} camera={{position:[0,-.10,8.8],fov:38,near:.1,far:40}} gl={{antialias:true,alpha:false,powerPreference:"high-performance"}} fallback={<div style={{width:"100%",height:"100%",minHeight:650,display:"grid",placeItems:"center",background:"#020611",color:"#DDF7FF",fontFamily:"system-ui"}}>3D renderer unavailable</div>} onCreated={({gl})=>{gl.setClearColor("#020611",1);gl.toneMapping=THREE.ACESFilmicToneMapping;gl.toneMappingExposure=1.16;gl.outputColorSpace=THREE.SRGBColorSpace;gl.domElement.style.display="block";gl.domElement.style.background="#020611";}}>
   <Scene emotion={emotion} view={view} colorFlowSpeed={THREE.MathUtils.clamp(colorFlowSpeed,.01,.15)} orbColors={orbColors} state={state} audioLevel={THREE.MathUtils.clamp(audioLevel||0,0,1)} audioBands={audioBands} bow={bow} hat={hat} audio={audio} glasses={glasses}/>
  </Canvas>
 </div>;
}
